...
class CallHandler:
    ...
    async def handle_call(self, ctx: JobContext) -> None:
        ...
        try:
            ...
            async with measure_latency_context("session_creation", call_id):
                ...
                # Parse metadata to override prompt with campaign specifics and contact info
                try:
                    # Try job metadata first, then room metadata
                    meta_source = ctx.job.metadata or ctx.room.metadata
                    if meta_source:
                        job_meta = json.loads(meta_source)
                        
                        # 1. Campaign Prompt Override
                        campaign_prompt = job_meta.get("campaignPrompt")
                        if campaign_prompt:
                            logger.info(f"CAMPAIGN_PROMPT_INJECTED | length={len(campaign_prompt)}")
                            # Append campaign prompt to end of system prompt
                            current_prompt = assistant_config.get("prompt", "")
                            assistant_config["prompt"] = f"{current_prompt}\n\nCAMPAIGN INSTRUCTIONS:\n{campaign_prompt}"

                        # 2. Contact Info Context
                        contact_info = job_meta.get("contactInfo")
                        if contact_info:
                            c_name = contact_info.get("name")
                            c_email = contact_info.get("email")
                            c_phone = contact_info.get("phone")
                            
                            context_str = f"You are calling {c_name or 'the customer'}."
                            if c_email:
                                context_str += f" Their email is {c_email}."
                            
                            logger.info(f"CONTACT_INFO_INJECTED | name={c_name}")
                            
                            # Prepend context so it sets the stage
                            current_prompt = assistant_config.get("prompt", "")
                            assistant_config["prompt"] = f"CONTEXT: {context_str}\n\n{current_prompt}"
                ...
                if data_collection.get("collectName"):
                    collection_prompts.append("You MUST ask for and collect the user's Name.")
                
                if data_collection.get("collectEmail"):
                    collection_prompts.append("You MUST ask for and collect the user's Email address.")
                    
                if data_collection.get("collectPhone"):
                    collection_prompts.append("You MUST verify the user's Phone Number.")

                if collection_prompts:
                    current_prompt = assistant_config.get("prompt", "")
                    mandatory_instruction = "CRITICAL: " + " ".join(collection_prompts) + " You cannot proceed with assistance until you have these details."
    ...
    async def _save_call_history_to_database(
        ...
        try:
            ...
            # Extract contact name and email from agent's booking data if available
            client_name = ""
            client_email = ""
            
            try:
                # Try to get from agent's booking data
                if hasattr(agent, '_booking_data'):
                    if hasattr(agent._booking_data, 'name') and agent._booking_data.name:
                        client_name = agent._booking_data.name
                    if hasattr(agent._booking_data, 'email') and agent._booking_data.email:
                        client_email = agent._booking_data.email
            except Exception as e:
                pass
            ...
            if success:
                ...
                # Send post-call email if configured
                await self._send_post_call_email(call_sid or call_id, client_email, assistant_config)
    ...
    async def _send_post_call_email(
        self, 
        call_identifier: Optional[str], 
        client_email: Optional[str],
        assistant_config: Dict[str, Any]
    ) -> None:
        """Send post-call email with documents if configured."""
        try:
            # Check if we have the necessary data
            if not call_identifier:
                logger.debug("POST_CALL_EMAIL_SKIP | no call_identifier available")
                return
                
            if not client_email:
                logger.debug("POST_CALL_EMAIL_SKIP | no client email collected")
                return
            
            # Check if assistant has email template configured
            email_templates = assistant_config.get("email_templates", {})
            post_call_template = email_templates.get("post_call")
            
            if not post_call_template or not post_call_template.get("subject"):
                logger.debug("POST_CALL_EMAIL_SKIP | no email template configured")
                return
            
            # Check if email credentials are linked
            data_collection = assistant_config.get("dataCollectionSettings", {})
            linked_email_id = data_collection.get("linkedEmailId")
            
            if not linked_email_id:
                logger.warning("POST_CALL_EMAIL_SKIP | no email credentials linked")
                return
            
            logger.info(f"POST_CALL_EMAIL_TRIGGER | call_id={call_identifier} | recipient={client_email}")
            
            # Make HTTP request to backend to send email
            backend_url = os.getenv("BACKEND_URL", "http://localhost:4000")
            email_endpoint = f"{backend_url}/api/v1/calls/{call_identifier}/send-email"
            
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(email_endpoint)
                
                if response.status_code == 200:
                    result = response.json()
                    if result.get("success"):
                        logger.info(f"POST_CALL_EMAIL_SENT | call_id={call_identifier} | recipient={client_email} | attachments={result.get('attachmentCount', 0)}")
                    elif result.get("skipped"):
                        logger.debug(f"POST_CALL_EMAIL_SKIPPED | reason={result.get('message')}")
                    else:
                        logger.warning(f"POST_CALL_EMAIL_FAILED | reason={result.get('message')}")
                else:
                    logger.error(f"POST_CALL_EMAIL_ERROR | status={response.status_code} | response={response.text}")
                    
        except Exception as e:
            # Don't fail the call if email sending fails
            logger.error(f"POST_CALL_EMAIL_EXCEPTION | error={str(e)}")
