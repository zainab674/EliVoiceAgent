...
async function resolveAssistantId(ctx, { phoneNumber, assistantId }) {
  ...
  try {
    ...
    const id = mapping?.inboundAssistantId || null;
    log(ctx, 'assistantId: resolved from mongodb', { phoneNumber: toE164(phoneNumber), assistantId: id });
    return id;
  } catch (e) {
...
livekitSipRouter.get('/assistant/:id', async (req, res) => {
  ...
  try {
    ...
    console.log(`Searching for assistant with ID: ${id}`);
    ...
    const payload = {
      ...
      assistant: {
        ...
        llm_model_setting: assistant.llmModel || 'gpt-4o-mini',
        temperature_setting: assistant.temperature || 0.1,
        max_token_setting: assistant.maxTokens || 250,
      },
      calendar_integration_id: assistant.calendarId || undefined,
      cal_api_key: assistant.calApiKey || undefined,
      cal_event_type_id: assistant.calEventTypeId || undefined,
      cal_event_type_slug: assistant.calEventTypeSlug || undefined,
      cal_timezone: assistant.calTimezone || undefined,
      first_sms: assistant.firstSms || ''
    };
    log(ctx, 'assistant resolved', { id, hasPrompt: !!assistant.systemPrompt, hasFirst: !!assistant.firstMessage });
