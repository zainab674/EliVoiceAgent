class SMSAssistantService {
  ...
  async sendSMSResponse(toNumber, fromNumber, message, userId) {
    try {
      ...
      const credentials = await this.databaseService.getTwilioCredentials(userId);
      ...
      // Map Mongoose result to expected format if needed (Mongoose model uses camelCase, existing code expects snake_case/camelCase mix?)
      // The TwilioCredential model uses: accountSid, authToken, isActive, label
      // I should standardize or map here.
      ...
      console.log('Found Twilio credentials:', {
        id: mappedCredentials.id,
        account_sid: mappedCredentials.account_sid ? 'Set' : 'Not set',
        auth_token: mappedCredentials.auth_token ? 'Set' : 'Not set',
        is_active: mappedCredentials.is_active,
        label: mappedCredentials.label
      });
