...
export async function createMainTrunkForUser({ accountSid, authToken, userId, label }) {
  ...
  try {
    ...
    await TwilioCredential.updateMany(
      { userId, isActive: true },
      { isActive: false, updatedAt: new Date() }
    );

    // Then insert the new active configuration
    const newCredential = await TwilioCredential.create({
      userId,
      accountSid,
      authToken,
      trunkSid,
...
export async function attachPhoneToMainTrunk({ twilio, phoneSid, e164Number, userId, label }) {
  ...
  // 2) Get the user's main trunk SID from credentials (MongoDB)
  const credentials = await TwilioCredential.findOne({ userId, isActive: true });

  if (!credentials || !credentials.trunkSid) {
    throw new Error('No main trunk found for user. Please create Twilio credentials first.');
  }

  const trunkSid = credentials.trunkSid;
  ...
  // 5) Persist phone number info in database (MongoDB)
  await PhoneNumber.findOneAndUpdate(
    { number: e164 },
    {
      ...
      number: e164,
      label: label || null,
      inboundAssistantId: null, // Reset or set logic
...
export async function getUserTwilioConfig(userId) {
  const data = await TwilioCredential.findOne({ userId, isActive: true });

  if (!data) {
    console.error('Error getting user Twilio config: No credential found');
    return null;
  }
  ...
  return {
    ...
    trunk_sid: data.trunkSid,
    domain_name: data.domainName,
    sip_username: data.sipUsername,
    sip_password: data.sipPassword,
    credential_list_sid: data.credentialListSid,
    label: data.label,
    user_id: data.userId
  };
}
