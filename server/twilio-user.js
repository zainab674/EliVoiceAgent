...
twilioUserRouter.get('/credentials', async (req, res) => {
  try {
    ...
    res.json({ success: true, credentials: credentials || null });
  } catch (error) {
...
twilioUserRouter.get('/phone-numbers', async (req, res) => {
  try {
    ...
    const twilio = Twilio(credentials.accountSid, credentials.authToken);

    let all;
    try {
      all = await twilio.incomingPhoneNumbers.list({ limit: 1000 });
    } catch (twilioError) {
      console.error('Twilio API error:', twilioError);
      return res.status(500).json({ success: false, message: 'Failed to fetch phone numbers from Twilio' });
    }
    ...
    const rows = all.map((n) => {
      ...
      return { ...row, usage: classifyUsage(row, credentials.trunkSid) };
    });

    const unusedOnly = req.query.unused === '1';
    const strict = req.query.strict === '1';
    const filtered = unusedOnly
      ? rows.filter((n) => (strict ? isStrictlyUnused(n) : isUnusedForOurWebhook(n, credentials.trunkSid)) && !n.mapped)
      : rows;
...
twilioUserRouter.post('/trunk/attach', async (req, res) => {
  try {
    ...
    console.log(`Using trunk_sid: ${credentials.trunkSid} for user ${userId}`);
    ...
    try {
      const trunk = await twilio.trunking.v1.trunks(credentials.trunkSid).fetch();
    ...
    try {
      await twilio.incomingPhoneNumbers(phoneSid).update({
        trunkSid: credentials.trunkSid,
      });
      console.log(`Successfully attached phone ${phoneNumber.phoneNumber} to trunk ${credentials.trunkSid}`);
    } catch (attachError) {
...
twilioUserRouter.post('/create-main-trunk', async (req, res) => {
  try {
    ...
    // (Note: createMainTrunkForUser must be updated to use Mongoose if it hasn't been already)
