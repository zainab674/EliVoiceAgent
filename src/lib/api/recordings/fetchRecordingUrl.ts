...
export const fetchRecordingUrl = async (callSid: string): Promise<RecordingInfo | null> => {
  try {
    ...
    // Get Twilio credentials using the service
    ...
    const response = await fetch(
      `${import.meta.env.VITE_BACKEND_URL || import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/v1/call/${callSid}/recordings?accountSid=${encodeURIComponent(credentials.account_sid)}&authToken=${encodeURIComponent(credentials.auth_token)}`,
      {
        headers: {
    ...
    const proxyAudioUrl = `${import.meta.env.VITE_BACKEND_URL || import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/v1/call/recording/${recording.sid}/audio?accountSid=${encodeURIComponent(credentials.account_sid)}&authToken=${encodeURIComponent(credentials.auth_token)}`;
