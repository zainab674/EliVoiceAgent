    ...
    const loadAssistant = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Map Mongoose schema to flat structure for VoiceAgent usage
          // Mongoose: modelSettings { provider, model, ... }, voiceSettings { provider, ... }
          const mappedAssistant = {
            id: data._id,
            name: data.name,
            prompt: data.modelSettings?.customPrompt || data.systemPrompt,
            first_message: data.firstMessage,
            llm_provider_setting: data.modelSettings?.provider,
            llm_model_setting: data.modelSettings?.model,
            temperature_setting: data.modelSettings?.temperature,
            max_token_setting: data.modelSettings?.maxTokens,
            voice_provider_setting: data.voiceSettings?.provider,
            voice_model_setting: data.voiceSettings?.model, // or voiceId?
            voice_name_setting: data.voiceSettings?.voiceId || data.voiceSettings?.name, // naming variation
            background_sound_setting: data.voiceSettings?.backgroundSound,
            wait_seconds: data.voiceSettings?.secondsToWait, // standardizing
            smart_endpointing: data.voiceSettings?.smartEndpointing,
            // Assuming defaults if missing
            cal_api_key: data.advancedSettings?.calApiKey,
            cal_event_type_id: data.advancedSettings?.calEventTypeId,
            cal_event_type_slug: data.advancedSettings?.calEventTypeSlug,
            cal_timezone: data.advancedSettings?.calTimezone,
          };
          setAssistant(mappedAssistant);
        } else {
          console.warn("Failed to load assistant via API");
          setAssistant(null);
        }
      } catch (error) {
  ...
  const AutoMic: React.FC = () => {
    ...
    return (
      <div className="flex items-center gap-2 mt-3">
        <TrackToggle source={"microphone" as any}>
          {(props) => {
            const { enabled } = props as { enabled: boolean };
            return (
