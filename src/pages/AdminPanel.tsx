...
const AdminPanel = () => {
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/v1/admin/plans`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      toast.success(editingPlanKey ? 'Plan updated successfully!' : 'Plan created successfully!');
  ...
  const fetchUserStats = async (userId: string) => {
    try {
      ...
      if (json.success && json.data) {
        console.log('✅ Final stats:', json.data);
