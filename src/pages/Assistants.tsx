...
export default function Assistants() {
  ...
  const [activeTab, setActiveTab] = useState(tabFromUrl === 'phone-numbers' ? 'phone-numbers' : 'assistants');
  const [tabChangeTrigger, setTabChangeTrigger] = useState(0);

  // Update active tab when URL parameter changes
  useEffect(() => {
    if (tabFromUrl === 'phone-numbers') {
      setActiveTab('phone-numbers');
    } else if (tabFromUrl === 'assistants' || !tabFromUrl) {
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Manage your AI assistants and phone numbers
                  </p>
                </div>
              </div>
            </div>
