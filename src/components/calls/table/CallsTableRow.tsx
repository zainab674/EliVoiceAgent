    ...
    >
      <TableCell className={isBookedAppointment ? "font-medium text-primary" : "font-medium text-foreground"}>
        <Link to={`/calls/${call.id}`} className="hover:underline">
          <div className="flex items-center gap-3">
            {getCallIcon(call)}
            <div>
              <div className="text-base font-bold font-feature-settings tracking-tight antialiased">{getCustomerName(call)}</div>
              <div className="text-sm font-medium text-muted-foreground font-feature-settings tracking-tight">
      ...
      <TableCell>
        <div className="flex flex-col">
          <span className="text-base font-bold text-foreground font-feature-settings tracking-tight antialiased">{formattedDateTime.date}</span>
          <span className="text-sm font-medium text-muted-foreground font-feature-settings tracking-tight">{formattedDateTime.time}</span>
        </div>
      </TableCell>
      <TableCell>
        <div className="flex items-center text-foreground">
          <Clock size={16} className="mr-2 text-muted-foreground" />
          <span className="text-base font-bold font-feature-settings tracking-tight antialiased">{formatCallDuration(call.duration)}</span>
        </div>
      </TableCell>
      <TableCell>
        {call.call_recording && (
