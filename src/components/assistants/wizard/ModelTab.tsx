...
export const ModelTab: React.FC<ModelTabProps> = ({ data, onChange }) => {
  ...
  const fetchEventTypesForCalendar = async (calendarCredentialId: string) => {
    try {
      ...
      setEventTypesError(null);
      const fetchedEventTypes = await fetchEventTypesFromCalCom(calendarCredentialId);
      console.log("Raw fetched event types:", fetchedEventTypes);
      console.log("Is array?", Array.isArray(fetchedEventTypes));
      console.log("Length:", fetchedEventTypes?.length);
      setEventTypes(Array.isArray(fetchedEventTypes) ? fetchedEventTypes : []);
      console.log("Event types set in state:", Array.isArray(fetchedEventTypes) ? fetchedEventTypes : []);
    } catch (error) {
  ...
  // Fetch calendar credentials on component mount
  useEffect(() => {
