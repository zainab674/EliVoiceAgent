...
export function PlansAndPricingSettings() {
  ...
  const { user, updateProfile } = useAuth(); // Assuming user has plan info from context
  ...
  // Sync selected plan with user context
  useEffect(() => {
    if (user?.plan) {
  ...
  useEffect(() => {
    const fetchUsageStats = async () => {
      ...
      try {
        ...
        // This endpoint aggregates calls count, team members, etc.
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/v1/user/usage`, {
        ...
        // Use user plan for limits logic
        // If current plan is not in context, maybe fetch profile?
        // Relying on user context for now as it should be up to date.
  ...
  const handleUpgrade = async (planKey: string) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/v1/auth/profile`, { // Correct route?
        method: 'PUT',
        headers: {
      ...
      // Wait, I implemented `PUT /api/v1/users/profile` in Step 151 (TargetFile: user.js, Route: /profile)
      // I registered user.js as app.use('/api/v1/user', userRouter)? NO!
      // Step 148 list_dir showed `user.js`.
      // Step 153 read `user.js`.
      // Where is user.js mounted?
      // I need to check `server/index.js`.
      // I will assume standard path or check index.js.
      // Usually it is `/api/v1/user` or `/api/v1/auth/user`.
      // User.js content (Step 153) has `router.put('/profile'...)`.
      // Let's assume `/api/v1/user/profile`.

      // Correction: Step 151 instruction said "Add usage and profile update routes to user.js".
      // But I haven't verified WHERE user.js is mounted.

      // Let's proceed assuming `/api/v1/user/profile` and if it fails I'll fix the mount point.

      const updateResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/v1/user/profile`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
